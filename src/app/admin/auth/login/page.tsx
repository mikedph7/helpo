  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user.role !== 'ADMIN') {
        setError('This account does not have admin privileges. Please contact your system administrator.');
        setIsLoading(false);
        return;
      }
      ...
      <Card className="w-full max-w-md border-red-200 shadow-lg">
        <CardHeader className="space-y-1">
          <div className="flex items-center justify-center mb-4">
            ...
            <span className="text-lg font-medium text-red-600 ml-2">admin</span>
          </div>
          <CardTitle className="text-2xl text-center text-red-700">Admin Portal</CardTitle>
          <p className="text-gray-600 text-center">Restricted access - authorized personnel only</p>
        </CardHeader>
        <CardContent>
